...
#     def validate(self, data): # this is build in fuction for object validation
#         """ check that description and title are different 
#         https://www.django-rest-framework.org/api-guide/serializers/#object-level-validation
#         """
#         if data["title"] == data["description"]:
...
#     def validate_title(self, value): # for field validation formate <validate>_<field name>
...
#     def validate_author(self, value): # for field validation formate <validate>_<field name>
#         if len(value) < 3:
#             raise serializers.ValidationError("Auther name shoud be at least 3 character")
#         return value
...
#     time_since_publication = serializers.SerializerMethodField() # add aditional field in to database
#     #author = JournalistSerializer(read_only=True) 
#     #author = serializers.StringRelatedField()
...
# class JournalistSerializer(serializers.ModelSerializer):
#     article = ArticleSerializer (many=True,read_only=True) #many=True use for whole query set
#     class Meta:
#         model = Journalist
#         fields = "__all__"
